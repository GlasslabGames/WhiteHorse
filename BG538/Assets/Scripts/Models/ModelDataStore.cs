...
public static class ModelDataStore {
	...
	} // the previous function will automatically be called if this is the first time SetUp was called
				...
				if (list != null) {
					...
					if (listType == typeof(System.Int64) || listType == typeof(System.Int32)) {
						...
						if (propertyInfo != null) {
							...
							fieldInfo.SetValue(obj, intList);
						}
							...
							if (propertyInfo != null) {
								propertyInfo.SetValue(obj, floatList, null);
							} else {
								fieldInfo.SetValue(obj, floatList);
							}
								...
								if (propertyInfo != null) {
									...
									fieldInfo.SetValue(obj, stringList);
								}
							} else {
								Debug.LogError("[ModelDataStore] Property " + key + " has list with unusable type " + listType);
							}
				} else {
					Debug.LogError("[ModelDataStore] Property " + key + " has unusable generic type " + node[key].GetType());
				}
			} else {
