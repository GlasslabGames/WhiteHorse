...
public class State : MonoBehaviour {
	...
	public float PlayerSupportPercent {
		get {
			if (GameObjectAccessor.Instance.Player.IsRed) {
				return RedSupportPercent;
			} else {
				return BlueSupportPercent;
			}
		}
		set {
			if (GameObjectAccessor.Instance.Player.IsRed) {
				RedSupportPercent = value;
			} else {
				BlueSupportPercent = value;
			}
		}
	}
	...
	void Awake() {
		...
		if (m_stateStripes == null) {
			Debug.LogError("No stripes on " + this, this);
		}
		if (m_stateOutline == null) {
			Debug.LogError("No outline on " + this, this);
		}
	...
	public bool SendOpponentPlayerSupporters() {
		if (m_playerSupportersSentToOpponent) {
	...
	public float GetPlayerPercentChange() {
		float change = 0;
		for (var i = 0; i < m_playerSupporterList.Count; i++) {
			CampaignWorker worker = m_playerSupporterList[i].GetComponent< CampaignWorker >();
			change += worker.PercentChange;
		}
		// Note, we might want to just multiply the number of supporters by the percent change when we simplify this stuff
		return change;
	}

	public float GetOpponentPercentChange() {
		float change = 0;
		for (var i = 0; i < m_opponentSupporterList.Count; i++) {
	...
	public bool PlayerIncrement() {
		if (m_currentPlayerSupporterIteration >= m_playerSupporterList.Count) {
			return UpdatePopularVotePlayer();
		}
    ...
    */
	...
	public bool UpdateStateWithPopularVote() {
		...
		if (!m_stateUpdatedWithPopularVote) {
			Leaning newLeaning;
			if (IsRed) {
				newLeaning = Leaning.Red;
			} else if (IsBlue) {
					newLeaning = Leaning.Blue;
				} else {
					newLeaning = Leaning.Neutral;
				}
	...
	public void Highlight(bool active) {
		if (GameObjectAccessor.Instance.GameStateManager.CurrentTurnState == TurnState.ConnectPlayers) {
			return;
		}
	...
	public bool PlayerCanPlaceSupporter() {
		return (InPlay &&
			GameObjectAccessor.Instance.GameStateManager.CurrentTurnState == TurnState.Placement &&
			GameObjectAccessor.Instance.Budget.IsAmountAvailable(GameMove.GetCost(GameActions.NEW_SUPPORTER)));
		// && m_playerSupporterList.Count < UnitCap); // for now we're not capping supporters per state
	}
        
	public void PlayerPlaceSupporter() {    
		if (PlayerCanPlaceSupporter()) {
			Debug.Log("placing supporter!");
	...
	public bool PlayerCanRemoveSupporter() {
		return (InPlay &&
			GameObjectAccessor.Instance.GameStateManager.CurrentTurnState == TurnState.Placement &&
			m_playerSupporterList.Count > 0);
	}

	public void PlayerRemoveSupporter() {
		if (PlayerCanRemoveSupporter()) {
